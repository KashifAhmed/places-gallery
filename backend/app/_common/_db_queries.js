...
module.exports = function(app, db) {
    ...
    // ===================================
    // Push in array Item
    app.services._findAndPush = function(_query, _updateObject, collection_name, cb) {
        console.log("\x1b[34m", "------------------------------------");
        console.log("== > Method", "app.services.findAndPush");
        console.log('== > DateTime', new Date());
        console.log("== > data", JSON.stringify(_query));
        console.log("== > UpdateObject", JSON.stringify(_updateObject))
        console.log("== > collection_name", collection_name);
        ...
        if (_query && _updateObject && collection_name) {
            var COLLECTION = db[collection_name];
            COLLECTION.findOneAndUpdate(_query, { $push: _updateObject }, { safe: true, upsert: true, new: true }, function(error, docs) {
                if (error) {
                    cb(error, null);
                } else {
                    cb(null, docs);
    ...
    // ===================================
    // Pull form Array
    app.services._findAndPull = function(_query, _updateObject, collection_name, cb) {
        console.log("\x1b[34m", "------------------------------------");
        console.log("== > Method", "app.services.findAndPull");
        console.log('== > DateTime', new Date());
        console.log("== > data", JSON.stringify(_query));
        console.log("== > UpdateObject", JSON.stringify(_updateObject))
        console.log("== > collection_name", collection_name);
        ...
        if (_query && _updateObject && collection_name) {
            var COLLECTION = db[collection_name];
            COLLECTION.findOneAndUpdate(_query, { $pullAll: _updateObject }, function(error, docs) {
                if (error) {
                    cb(error, null);
                } else {
                    cb(null, docs);
